...
class _Setup(hierarchy._Hierarchy):
    ...
    def _input_attr_build_data(self):
        ...
        node_data.extend_attr_data(
            component_data.AttrData(
                self._LOC_SCALE, type_="double", parent=self._IN, value=1, min=0.1
            ),
            component_data.AttrData(
    ...
    def __setup_wire(
        ...
        if len(output_xforms.keys()) <= 1:
            return
        # creating wires
        hier_wire = nw.wrap_node(
            cmds.curve(
                name="xformWire",
                degree=1,
                point=[[0, x, 0] for x in output_xforms.keys()],
            )
        )
        ...
        hier_parent_wire = nw.wrap_node(
            cmds.curve(name="hierParentWire", degree=1, point=[[0, 0, 0], [0, 1, 0]])
        )
        ...
        (
            parent_init_mat["inputMatrix"]
            << self.container_node[self.HIER_DATA.HIER_PAR_INIT_INV_MAT]
        )

        # creating point mult for hier parent
        wire_points = []
        wire_points.append(nw.create_node("pointMatrixMult", "hierParent_pntMatMult"))
        ...
        for index, xform in output_xforms.items():
            ...
            if connection is not None:
                wire_points.append(
                    nw.create_node("pointMatrixMult", f"xform{index}_pntMatMult")
                )
                ...
                if index == 0:
                    (
                        wire_points[-1]["output"]
                        >> hier_parent_wire_shape["controlPoints"][1]
                    )
    ...
    def _override_build(
        ...
        **kwargs,
        ...
        for index, input_xform in input_xforms.items():
            # creating control
            control_inst = control.Locator.create(
            ...
            (
                control_container[control_inst._LOC_SCALE]
                << self.input_node[self._LOC_SCALE]
            )
            utils.Matrix.translate_matrix(0, index * 1.5, 0).set_transform(
                control_inst.transform_node, world_space=True
            )
            if input_xform.init_matrix.value is not None:
                utils.Matrix(input_xform.init_matrix).set_transform(
                    control_inst.transform_node, world_space=True
                )
...
class SimpleLimb(_Setup):
    ...
    def _override_build(
        ...
        **kwargs,
        ...
        control_inst0 = control.Locator.create(
            instance_name=self.input_node[self.HIER_DATA.IN_XFORM][0][
        ...
        (
            control_inst0.container_node[control_inst0._LOC_SCALE]
            << self.container_node[self._LOC_SCALE]
        )
        ...
        if input_xforms[0].init_matrix.value is not None:
            input_xforms[0].init_matrix.value.set_transform(
                control_inst0.transform_node, world_space=True
            )
        # control 1
        control_inst1 = control.Locator.create(
            instance_name=self.input_node[self.HIER_DATA.IN_XFORM][1][
                self.HIER_DATA.IN_XFORM_NAME
            ],
            parent=self,
            color=control_color,
        ...
        (
            control_inst1.container_node[control_inst1._LOC_SCALE]
            << self.container_node[self._LOC_SCALE]
        )
        ...
        if input_xforms[2].init_matrix.value is not None:
            ...
            control_inst2_translate.transform_node["translate"] = utils.Matrix(
                init_matrix
            ).translate
        (
            control_inst2_orient.container_node[control_inst2_orient._LOC_SCALE]
            << self.container_node[self._LOC_SCALE]
        )
        ...
        xform0_ws_mat_attr, xform0_inv_attr = self.__create_xform0(
            guide_transform0_ws_mat=control_inst0.container_node[
                control._Control._OUT_WS_MAT
            ],
            guide_transform1_ws_mat=control_inst1.container_node[
                control._Control._OUT_WS_MAT
            ],
        )
        ...
        xform1_ws_mat_attr, xform1_inv_attr = self.__create_xform1(
            ...
            guide_transform2_ws_mat=control_inst2_translate.container_node[
                control._Control._OUT_WS_MAT
            ],
    ...
    def __create_mid_interp_matrix(
        self, source_matrix: nw.Attr, target_matrix: nw.Attr
    ):
        ...
        translate_row_nodes = [
            nw.create_node("rowFromMatrix", "xform0_ws_translate"),
        ...
        self.container_node.add_nodes(
            aim_matrix, *translate_row_nodes, mid_matrix_inbetween_4x4, *average_nodes
        )
        return mid_matrix_inbetween_4x4["output"], translate_row_nodes[1]

    def __create_xform0(
        self, guide_transform0_ws_mat: nw.Attr, guide_transform1_ws_mat: nw.Attr
    ):
    ...
    def __create_xform1(
        self,
        guide_transform1_ws_mat: nw.Attr,
        guide_transform2_ws_mat: nw.Attr,
        xform0_ws_mat: nw.Attr,
        xform0_inv_mat: nw.Attr,
    ):
    ...
    def __create_xform2(
        self,
        xform1_ws_mat: nw.Attr,
        xform2_translate: nw.Node,
        xform2_orient: nw.Node,
        xform1_inv_mat: nw.Attr,
    ):
        ...
        self.container_node.add_nodes(
            xform_matrix2_ws, xform_matrix2_loc, xform2_mult_mat
        )
...
class Mirror(_Setup):
    ...
    def _input_attr_build_data(self):
        ...
        node_data.extend_attr_data(
            component_data.AttrData(
                self._IN_MIRROR_AXIS,
                type_=component_enum_data.AxisEnum.x,
                parent=self._IN,
            ),
            component_data.AttrData(
                self._IN_AXIS_SCALAR,
                type_="float",
                value=1,
                locked=True,
                parent=self._IN,
            ...
            component_data.AttrData(
                self._IN_MIRROR_MAT, type_="matrix", parent=self._IN
            ),
        )
        return node_data
        ...
        for axis_enum_index, axis in enumerate(component_enum_data.AxisEnum):
            ...
            for axis_index, choice_node in enumerate([x_choice, y_choice, z_choice]):
                if abs(axis_vec[axis_index]) < 0.001:
                    (
                        choice_node["input"][axis_enum_index]
                        << self.container_node[self._IN_AXIS_SCALAR]
                    )
                else:
                    (
                        choice_node["input"][axis_enum_index]
                        << self.container_node[self._IN_NEG_AXIS_SCALAR]
                    )
