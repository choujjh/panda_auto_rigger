...
class Twist(base_comp.Matrix):
    ...
    _IN_WRLD_INIT_MATRIX = "worldInitMatrix"
    _IN_WRLD_INIT_PAR_INV_MATRIX = "worldInitParentInvMatrix"
    ...
    _KWG_WRLD_INIT_MAT = "world_init_matrix"
    _KWG_WRLD_INIT_PAR_INV_MAT = "world_init_parent_inverse_matrix"
    ...
    def _input_attr_build_data(self):
        ...
        node_data.extend_attr_data(
            component_data.AttrData(self._IN_WRLD_INIT_MATRIX, type_="matrix", parent=self._IN),
            component_data.AttrData(self._IN_WRLD_INIT_PAR_INV_MATRIX, type_="matrix", parent=self._IN),
    ...
    def create(cls, 
               ...
               world_init_matrix:nw.Attr=None, 
               world_init_parent_inverse_matrix:nw.Attr=None,
               primary_vec=None):
        pre_build_kwargs, build_kwargs, post_build_kwargs = cls._process_kwargs(
            ...
            world_init_matrix=world_init_matrix, 
            world_init_parent_inverse_matrix=world_init_parent_inverse_matrix,
    ...
    def _process_kwargs(cls,
        ...
        pre_build_kwargs.update({
            ...
            cls._KWG_WRLD_INIT_MAT: world_init_matrix,
            cls._KWG_WRLD_INIT_PAR_INV_MAT: world_init_parent_inverse_matrix,
    ...
    def _pre_build(self, 
        ...
        utils.set_connect_attr_data(self.container_node[self._IN_WRLD_INIT_MATRIX], world_init_matrix)
        utils.set_connect_attr_data(self.container_node[self._IN_WRLD_INIT_PAR_INV_MATRIX], world_init_parent_inverse_matrix)
    ...
    def _override_build(self, **build_kwargs):
        ...
        world_init_matrix = self.container_node[self._IN_WRLD_INIT_MATRIX]
        world_init_par_inv_matrix = self.container_node[self._IN_WRLD_INIT_PAR_INV_MATRIX]

        if not loc_init_matrix.has_src_connection():
            # make a new local matrix
            if not world_init_matrix.has_src_connection() or not world_init_par_inv_matrix.has_src_connection():
                raise RuntimeError(f"{loc_init_matrix} does not have connection and ({world_init_matrix} or {world_init_par_inv_matrix}) does not have connection. loc_init_matrix could not be calculated")
            loc_init_matrix_mult = nw.create_node("multMatrix", "locInitMatrixMult")
            loc_init_matrix_mult["matrixIn"][0] << world_init_matrix
            loc_init_matrix_mult["matrixIn"][1] << world_init_par_inv_matrix
            loc_matrix = loc_init_matrix_mult["matrixSum"]
        ...
        no_rot_inverse = nw.create_node("inverseMatrix", "noRotInverseMatrix")
        no_rot_inverse["inputMatrix"] << aim_no_rot_matrix["outputMatrix"]

        rot_matrix_mult = nw.create_node("multMatrix", "rotMatrixMult")
        rot_matrix_mult["matrixIn"][0] << loc_matrix
        rot_matrix_mult["matrixIn"][1] << no_rot_inverse["outputMatrix"]

        self.container_node[self._OUT_ROT_MATRIX] << rot_matrix_mult["matrixSum"]
        
        self.container_node.add_nodes(aim_no_rot_matrix, no_rot_inverse, rot_matrix_mult)